...
class App: A by FirstDelegate(),B by SecondDelegate(){
    ...
    myPhone.makeCall()     // 📞 Calling... Battery: 50%
    myPhone.charge()       // 🔋 PowerBank: Fully charged!
    myPhone.makeCall()     // 📞 Calling... Battery: 100%
...
class PowerBank : Battery {
    ...
    override fun charge() {
        level = 100
        println("🔋 PowerBank: Fully charged!")
    }
...
class Phone(battery: Battery) : Battery by battery {
    fun makeCall() {
        if (getLevel() > 20) {  // Uses DELEGATED getLevel()
            println("📞 Calling... Battery: ${getLevel()}%")
        } else {
